...
12. **Human-only Project surfaces:** views, workflows, Insights, status updates, Ready prioritization / product roadmap — agents never edit these. **Desired-state schema** (`.ai_infra/templates/project-board/board-shell.schema.yaml`, optional overlay `.local/user_settings/board-shell.schema.yaml`) drives `project board-bootstrap --check` and the first-run coach (`.cursor/skills/board-shell-onboard/SKILL.md`) — check/coach only; **no** view create/rename via API. Opt-in exceptions (explicit flags, user-approved): `board-bootstrap --ensure-fields` (create missing field **definitions**) and `board-bootstrap --apply-readme` (`updateProjectV2` README). Human paste-pack: `views-setup.md` / `views-checklist.md` / `project-readme.md`.
...
15. **Tier-1 board fields:** agents set **Priority/Size/Estimate** on create/own (Size↔Estimate **points** rubric in `project-board-ssot` skill). **Start date** (UTC) is set when Status becomes `in_progress` if empty — via `claim`, `set-status --to in_progress`, or `handoff --to in_progress` — when `conventions.set_start_date_on_claim` and `fields.start_date.field_id` are configured. PR linkage via `mention-pr --pr N`. **Out of scope for agents by default:** Iteration, Labels, Reviewers, End date (human / UI).
