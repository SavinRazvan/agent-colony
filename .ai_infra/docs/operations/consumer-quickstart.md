...
| **4. Board shell** *(when `project_ssot.enabled`)* | Run `project doctor` (expect **ok**) then `board-bootstrap --check`. **Brand-new Project:** expect **FAIL** missing six views / `View 1` WARN — **normal** (no view-create CLI today). Agent chat **`/project-board`**: **CONSENT GATE** (description + proceed) then **TURN PROTOCOL** (kit README §4) → you click in GitHub UI → optional `--ensure-fields` / `--apply-readme` after `yes` → re-run `--check` until green → `project status`. **Not** first-run: `/enterprise-auditor`. |
