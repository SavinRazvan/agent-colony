...
| 4. Doctor + board shell | `python3 -m cursor_workflow project doctor` (expect **ok**) → `python3 -m cursor_workflow project board-bootstrap --check`. **New Project:** `FAIL — missing minimum view '…'` / `WARN — rename … 'View 1'` is expected — **not** a README-only problem unless FAIL lines say so. → Agent chat **`/project-board`** + `.cursor/skills/board-shell-onboard/SKILL.md` → **humans:** follow `.ai_infra/templates/project-board/views-setup.md` (six Playground views + Tier-1 columns) and paste **contents of** `project-readme.md` (or `--apply-readme`) → re-run `board-bootstrap --check` until green → `python3 -m cursor_workflow project status`. |
