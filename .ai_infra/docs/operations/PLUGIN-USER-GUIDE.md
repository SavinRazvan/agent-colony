...
| **Runtime** | `.local/`, `.venv` | No | Trackers, local artifacts, user settings (gitignored) |
...
| 6. Rate-limit buffer | Gate: `project api-ready` → (if yes) `project entry` / writes / `outbox flush`. Writes may **precheck** GraphQL quota (cached REST) or queue on throttle / Forbidden / 429. If a write returns **EXIT_QUEUED (6)**, do **not** retry-loop — check `cooldown status` / `outbox status`; triage pending/failed with `outbox list` and `outbox drop --id … --force` (smoke/stale only); after quota recovers run `project api-ready` then `outbox flush`. Configure `project_ssot.outbox` (`precheck_writes`, `dedupe_pending`, …) in collaboration YAML. Outbox is a local buffer, not a second Status SSOT. Prefer bare `project_ssot.owner` login (no `users/`\|`orgs/` prefix — doctor WARNs URL-shaped YAML). |
...
│   └── workflows/
├── .local/                        # trackers and local artifacts (gitignored)
...
| **Terminal** | `python3 -m agent_colony …`, pytest, local verification |
...
5. Optional: [Canvases and offline markdown](#5-canvases-and-offline-markdown)
...
3. **`/prepare-pr`** (runs `prepare.py` **`resolve_gates()`** — **2** checks on consumer, **6** on kit-dev)
