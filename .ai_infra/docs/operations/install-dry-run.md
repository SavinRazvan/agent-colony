...
Re-activate is idempotent: existing trackers and `AGENTS.md` are not overwritten.
...
- [ ] Sixteen canonical skills under `.cursor/skills/` (includes `board-shell`, `agent-surface-parity`)
...
|-------|----------|---------|
| `prepare.py` / `resolve_gates()` count | **2** universal | **6** (auto-appends drift + doc facts + check-plugin + check_audit_artifacts) |
...
| `agent_colony_mcp` / `workflow_gate_count` (`load_gates` → `resolve_gates()`) | **2** | **6** |
