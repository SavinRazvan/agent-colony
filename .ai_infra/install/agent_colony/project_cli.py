"""
File: project_cli.py
Path: .ai_infra/install/agent_colony/project_cli.py
...
Depends On:
 ...
 - .ai_infra/install/agent_colony/project_parser.py
...
def cmd_guide(args: argparse.Namespace) -> int:
    ...
    print(
        "python3 -m agent_colony project entry  "
        "# quota-aware Entry (live|conserve|offline_artifacts); prefer over unfiltered list"
    )
    print(
        "python3 -m agent_colony project export --reuse-if-fresh 900  "
    ...
    print(
        f"python3 -m agent_colony project handoff --last --agent {agent} "
        f"--next {nxt} --to in_review  "
        "# EXIT_VALIDATION (5) if Acceptance/Rollback still (TBD)"
    )
    print(
        f"python3 -m agent_colony project mention-pr --pr <n> --last --agent {agent}  "
