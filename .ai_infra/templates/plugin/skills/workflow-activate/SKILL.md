...
5. When `project_ssot.enabled`: first-run board shell — **`/project-board`** + `board-shell` **CONSENT GATE** then TURN PROTOCOL → `project board-bootstrap --check` (optional `--ensure-fields` / `--apply-readme`) → `project status`. **Do not** start with `/auditor`.
