...
1. **SSOT select:** `python -m cursor_workflow project status`. If operational → list/claim board card (`claim --last` after create, or claim Ready). When creating: see `.ai_infra/templates/project-board/README.md` — `create-from-template --template slice` (feature/`chore/`) or `--template bug` (defect/`fix/`), then `claim --last --agent implementer`, then fill **Tier-1 fields** (skill § Tier-1 card fields contract): `set-field --field priority --to p0|p1|p2`, `set-field --field size --to xs|s|m|l|xl`, `set-field --field estimate --to N` (defaults `s` / `1` if unknown). Confirm Status + Start date from claim. Else read plan + tracker; one task `in_progress` locally; tag tasks `[P0]`…`[P3]` in the plan note.
