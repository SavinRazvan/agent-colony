...
- [ ] If EXIT_QUEUED: `outbox status` / `outbox list` / `cooldown status`; drop smoke/stale with `outbox drop --force`; purge tombstones with `outbox purge --status failed --force`; no API hammering
