"""
File: test_agent_colony_cli_full.py
Path: tests/modules/install/test_agent_colony_cli_full.py
Role: Full-branch in-process coverage for .ai_infra/install/agent_colony/cli.py.
Used By:
 - pytest
Depends On:
...
def test_cmd_health_pass_without_mcp_user_fragment(
    ...
    (tmp_path / ".ai_infra" / ".kit-version").write_text("0.6.4\n", encoding="utf-8")
...
def test_build_parser_version_exits(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "agent-colony 0.6.5" in capsys.readouterr().out
