...
2. Alignment files when architecture-impacting (authored via **`auditor`** / `auditor-protocol` skill; see `advisory-audit-alignment-enforcement.mdc`): both `.local/workflow-artifacts/alignment/alignment-audit.md` and `alignment-todos.md` must exist, carry `Audit-Schema: 1`, and **PASS** `python .ai_infra/scripts/workflow/check_audit_artifacts.py --arch-impacting --summary` (or MCP `workflow_check_audit_artifacts`) before `prepare.py`. No open **P0/P1** (`status: open` fails Schema-1). Pipeline `architecture_impacting` / `requires_alignment_artifacts` **and** kit-dev path-trigger (`arch_impacting_paths.py`) **enforce** this at merge (not PR-body hint only). (Default 6th prepare gate still skips unstamped leftover files.)
