...
- **Linked Issue closure (opt-in):** when `conventions.close_linked_issue_on_cleanup: true`, `finalize.py` calls `project close-linked-issue --pr <n>` after branch cleanup succeeds. This closes the GitHub Issue linked to the merged PR's board item — never wired to `set-status`/`claim`/`handoff` so it can't race ahead of merge/cleanup evidence. Outcomes are recorded in `finalize.md § Linked Issue Closure` as `PASS` / `SKIPPED` (no linked Issue, already closed, or flag disabled) / `DEFERRED` (gh API error — non-blocking). Rationale: board `Status=Done` and the underlying GitHub Issue `open`/`closed` state are otherwise independent (ADR-008) — this closes that gap only when a maintainer opts in.
