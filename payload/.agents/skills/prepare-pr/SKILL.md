...
2. Alignment files when architecture-impacting (authored via **`auditor`** / `auditor-protocol` skill; see `advisory-audit-alignment-enforcement.mdc`): both `.local/workflow-artifacts/alignment/alignment-audit.md` and `alignment-todos.md` must exist, carry `Audit-Schema: 1`, and **PASS** `python .ai_infra/scripts/workflow/check_audit_artifacts.py --arch-impacting --summary` before `prepare.py`. No open **P0** unless explicitly accepted. (Default 6th prepare gate still skips unstamped leftover files.)
