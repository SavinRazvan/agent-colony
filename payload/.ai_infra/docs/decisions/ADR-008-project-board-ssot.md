...
12. **Human-only Project surfaces:** views, workflows, Insights, Project README, status updates, Ready prioritization / product roadmap — agents never edit these. Read-only probes such as `project board-bootstrap --check` and `project doctor` are allowed to inspect readiness, but they must not mutate views/README/workflows. Human paste-pack templates live in `.ai_infra/templates/project-board/views-setup.md` and `views-checklist.md`.
