...
| **`make drift-validate`** | Slice closure / maintainer hygiene | Operational drift (**DRIFT-001…017** + 004b / 011b on kit-dev; consumer profile subset). See [token-efficiency-enforcement.md](token-efficiency-enforcement.md) · [ADR-007](../decisions/ADR-007-workflow-drift-guard.md) catalog | `.ai_infra/scripts/workflow/check_drift.py` |
| **Consumer drift** | Post-install verify on app projects | `drift validate --profile consumer` — DRIFT-005 (skip when `IMPLEMENTATION-STATUS.md` absent) + DRIFT-008 + DRIFT-013 + DRIFT-011b + **DRIFT-014** + **DRIFT-016** | [consumer-quickstart.md](consumer-quickstart.md#drift-on-consumer-apps) |
