...
Path: .ai_infra/install/cursor_workflow/activate_cli.py
...
def _print_post_activate_hints(root: Path) -> None:
    ...
    if collab_enabled:
        ...
        print("  4. Run: source .venv/bin/activate && python3 -m cursor_workflow project doctor")
        print("  5. Agent chat: /project-board — CONSENT GATE (description + proceed) then TURN PROTOCOL coach")
        print("  6. Run: source .venv/bin/activate && python3 -m cursor_workflow project board-bootstrap --check")
        print("     Until shell green (kit default six views, or minimal 2-view overlay; Tier-1 columns on Status board + Prioritized backlog).")
        print("  7. Run: source .venv/bin/activate && python3 -m cursor_workflow project status")
        print("  8. In Agent chat: /implementer — local trackers are offline fallback under board_only.")
    else:
        print("\nYou're almost done — 3 quick steps:")
        print(f"  1. Edit {yaml_path}")
        print('     Set owner.display_name and owner.github_user (replace placeholders).')
        print("  2. Run: source .venv/bin/activate && python3 -m cursor_workflow contributors validate")
