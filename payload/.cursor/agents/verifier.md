...
**Tier-1 fields (mandatory):** On create/claim/own fill Status, Priority, Size, Estimate, Start date (via `claim`), Assignee (human — promote Draft→Issue first if claim WARNs), and Linked PR via `mention-pr` when a PR exists. `set-field --field priority --to p0|p1|p2`; `size` → `xs|s|m|l|xl` (default `s`); `estimate` → N (default `1`). Chat **P3**/deferred → board `p2` + Notes `deferred`. Exit: `Priority=p? · Size=? · Estimate=?` and `Tasks: [P0]…; [P1]…; [P2]…; [P3]…`. Canon: `.cursor/skills/project-board-ssot/SKILL.md` § Tier-1 card fields contract. On closure, spot-check Status/Priority/Size/Estimate/Start date; Assignee when Issue-backed; Linked PR when a PR was opened.
